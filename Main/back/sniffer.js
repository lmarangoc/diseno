  ...
  connection.query(query, function (err, results) {
    ...
    if (results) {
      if (
        ...
        query = `INSERT INTO Locations (latitude,longitude,fecha, conductor) values (${num_latitud},${num_longitud},${fecha},${conductor})`;
        connection.query(query, function (error, res) {
          ...
          if (res) {
            console.log("Save -> Conductor: " + cond);
          }
        });
      }
    }
...
server.post("/history/:cond", (req, res) => {
  ...
  const query = `select * from Locations where fecha >= ${ti} and fecha <= ${tf} and conductor = ${cond}`;
...
server.post("/interval/:cond", (req, res) => {
  const { r, lat_c, lng_c, ti, tf } = req.body;

  const { cond } = req.params;

  const query = `select * from Locations where fecha >= ${ti} and fecha <= ${tf} and conductor = ${cond} and (latitude - (${lat_c}))*(latitude - (${lat_c})) + (longitude - (${lng_c}))*(longitude - (${lng_c})) < ${r}`;
