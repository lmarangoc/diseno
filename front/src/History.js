...
function getRadio(center_t, radio, data) {
  const k = [];
...
export const History = () => {
  ...
  const [center_t, setCenter_t] = useState({ lat: 0, lng: 0 });
  ...
  const onClick = useCallback(
    (e) => {
      const center_tmp = { lat: e.latlng.lat, lng: e.latlng.lng };

      setCenter_t(center_tmp);

      const k = getRadio(center_tmp, radio, data);
      setData2(k);
  ...
  useEffect(() => {
    if (data !== null) {
      const k = getRadio(center_t, radio, data);
      setData2(k);
    }
  }, [radio, center_t, data]);
    ...
    <div className="container">
      ...
      <div className="wrapper">
        <div className="options-h" style={{ width: optionWidth.op }}>
          ...
          {<List data={data2} onUpdate={onUpdate} />}
        </div>
        ...
        <div className="map-h" style={{ width: optionWidth.mp }}>
          <CustomMap
            ...
            poly3={mode === 0 ? poly : null}
            canMove={init}
            fixed={mode === 0 ? null : data4}
          />
        </div>
      </div>
    </div>
